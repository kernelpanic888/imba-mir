...
def _world_event(event: object | None) -> dict[str, Any] | None:
    ...
    return {
        ...
        "playerHealing": event.player_healing,
...
def snapshot(game: WorldGame) -> dict[str, Any]:
    ...
    return {
        ...
        "firstStrike": {
            ...
            "capacity": state.strike_preview.capacity,
        ...
        "lastBalanceCapacity": state.last_balance_capacity,
        "living": {
            "identity": state.seed,
...
def _calculation(
    ...
    if action == "first_strike":
        ...
        return common | {
            "scene": "attack",
            "relation": "c = min(12, ticks + τ + ρ)",
            "signals": [
                _signal("t", "ТИКИ", strike["confirmedTicks"]),
                _signal("τ", "НАПРЯЖЕНИЕ", strike["previousTension"]),
                _signal("ρ", "РЕФЛЕКСИЯ", strike["reflection"]),
                _signal("c", "ЁМКОСТЬ БАЛАНСА", strike["capacity"], "hold"),
            ],
            "title": "Сертификация инициативы баланса",
            "theorem": "initiative_capacity_bounded ∧ combat_admissible_iff",
            "equation": f"c = min(12, {strike['confirmedTicks']} + {strike['previousTension']} + {strike['reflection']}) = {strike['capacity']}",
            "steps": [
                _trace_step("GUARD", f"ticks>0 ∧ τ>0 ∧ unused = true"),
                _trace_step("BOUND", f"initiativeCapacity ↦ {strike['capacity']} ≤ 12"),
                _trace_step("APPEND", f"PLAYER/CONTACT · Σ{attack['epoch']} · parent h{attack['parentHead']}"),
                _trace_step("HOLD LIFE", "ΔHP Ворона = 0; ΔHP Мира = 0; ждём WORLD/REACTION", "wait"),
            ],
            "result": f"PENDING BALANCE {strike['capacity']}/12",
            "verdict": "APPEND / WAIT",
        }
    ...
    if action == "world_reaction":
        ...
        return common | {
            ...
            "relation": "lower(HP) + min(c, gap) → balance",
            "signals": [
                _signal("c", "ЁМКОСТЬ", event["power"], "hold"),
                _signal("P+", "ВОРОН ВОССТАНОВИЛ", event["playerHealing"]),
                _signal("W+", "МИР ВОССТАНОВИЛ", event["healing"]),
                _signal("Δ−", "УРОН ОБОИМ", 0, "ok"),
            ],
            "title": event["title"],
            "theorem": "direct_reaction_admitted ∧ balance_contact_has_no_damage",
            "equation": f"P + {event['playerHealing']}; W: {event['before']['life']} + {event['healing']} = {event['after']['life']}; damage = 0",
            "steps": [
                _trace_step("OBSERVE", f"CONTACT Σ{attack['epoch']} / h{attack['head']}"),
                _trace_step("COMPARE", "сравнить жизнь Ворона и Мира"),
                _trace_step("RESTORE", f"lower side: P+{event['playerHealing']} · W+{event['healing']}"),
                _trace_step("PRESERVE", "directDamage = 0 ∧ backlash = 0", "ok"),
                _trace_step("APPEND", f"WORLD/REACTION Σ{reaction['epoch']} → h{reaction['head']}"),
            ],
            "result": f"BALANCE · P {after['actors']['player']['life']} · W {event['after']['life']}",
            "verdict": reaction["verdict"],
        }
